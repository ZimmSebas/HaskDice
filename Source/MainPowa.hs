...
---------------------
  ...
  data State = S { inter :: Bool,       -- True, si estamos en modo interactivo.
                   ...
                   ve :: Env            -- Entorno con variables globales y su valor  [(Name, (Value, Type))]
                 }
  ...
  interpretCommand :: String -> IO Comm
  interpretCommand x
  ...
  handleCommand :: State -> Comm -> IO (Maybe State)
  ...
  data InteractiveCommand = Cmd [String] String (String -> Comm) String
  ...
  printStmt ::  Command -> IO ()
  printStmt comm = print comm  

  parseIO :: String -> (String -> ParseResult) -> String -> IO (Maybe a)
  parseIO f p x = case p x of
                       Left e  -> do putStrLn (f++": "++e) 
                                       return Nothing
